...
class TestSetup:
    ...
    async def test_setup_entry_restart_failure_creates_issue(
        self,
        hass: HomeAssistant,
        pr_response: dict[str, Any],
        tmp_path: Path,
    ):
        """Test that restart failure falls back to creating repair issue."""
        # Create entry with restart option
        entry = create_config_entry(
            hass,
            domain=DOMAIN,
            title="Test (PR #1)",
            data={
                ...
                CONF_REFERENCE_TYPE: ReferenceType.PR.value,
                CONF_REFERENCE_VALUE: "1",
                CONF_INTEGRATION_DOMAIN: "test_restart_fail",
                CONF_IS_PART_OF_HA_CORE: False,
            },
            unique_id="test_restart_fail",
        ...
        # Register a mock homeassistant.restart service that raises an exception
        async def mock_restart_service_fail(call):
            raise Exception("Restart failed")

        hass.services.async_register(
            "homeassistant", "restart", mock_restart_service_fail
        )
        ...
        assert result is True
        # Verify fallback to repair issue when restart fails
        mock_restart_issue.assert_called_once()
